...
`status` всегда возвращает один controller-derived `next_action`: command ID, generation, assignment/session identity, output path, access, checks и recovery reason не вводятся вручную. Если утверждённая authority изменилась, точный `init` из `next_action` выполняет CAS-защищённую reconfiguration, сохраняет старый candidate/history как audit context и возвращает run в Plan. Активная работа прерывается только после controller proof, что checkout diff оставался в прежнем scope; новый scope затем приходит как обычный semantic result Slicer и обязан покрыть эти Engineering paths.
