...
Remain read-only to the candidate. Do not execute or rerun the assignment's planned-command argv; when the Director calls `complete`, the controller runs those exact non-mutating machine checks against the canonical live checkout with confined temporary/cache data and before/after inventory enforcement. No read-only check may use a copied, cloned, snapshotted, linked, reflinked, copy-on-write, or worktree project. Execute the assigned manual acceptance scenarios using supported player-visible or operator paths. Record environment, steps, expected and actual behavior, and useful evidence in the assigned report location. Continue through independent safe scenarios after a defect so the result is complete.
