...
- `.agentic-pipeline/Workflows/<feature>/specification-state.json` for immutable same-feature `SPEC_READY` evidence;
- `.agentic-pipeline/Workflows/<feature>/development-plan-state.json` for operational planning state.

Every controller invocation requires the same lowercase `--feature`; both states bind exact `feature` and `workflow_path`. Planning reads runtime state only from `.agentic-pipeline/Workflows/<feature>/pipeline-state.json` and never scans, archives, moves, or deletes a sibling workflow.
...
- `### Context Capsule Budget` with all five positive numeric limits, `metric_scope: capsule_plus_referenced_files`, and bounded scalar `authority_paths`/`evidence_paths`; each is one comma-separated row, separator whitespace is ignored, and validation uses the runtime path grammar before submission; after approval the stable deduplicated union of authority entries followed by evidence entries is the pipeline controller's sealed per-slice read scope, so every entry must be a canonical project-relative exact path or terminal `dir/**` rule and must not use bare `**`, absolute/parent paths, or other glob syntax;
...
When current PRD/SPEC authority is unchanged but an approved plan requires revision, only `revise-approved --reopened-by <director-id> --analyst-id <fresh-analyst-id> --reason <exact reason>` may reopen it. The command reads only `.agentic-pipeline/Workflows/<feature>/pipeline-state.json`, validates its exact feature/workflow/project/plan binding, and never scans sibling folders. Schema-10 migration is unsupported by `git-tree-v1`; no retired import, reconstruction, mixed-lineage bridge, or recovery token grants continuation authority. Runtime reconfiguration carries no prior approval forward. Director and Analyst must remain distinct and the replacement draft must pass fresh analysis, validation, submission, and approval.
