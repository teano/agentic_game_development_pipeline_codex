...
An unavailable environment is a terminal `blocked` result with `user_input_required=true`, not a retry state. Archive the run and use a fresh `init` only after authority or capability evidence changes.
