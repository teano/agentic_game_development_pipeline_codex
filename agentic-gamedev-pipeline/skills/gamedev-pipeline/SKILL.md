...
Read current phase and `next_action` from controller state before every transition. Use current command `--help`; do not reconstruct signatures from memory.
...
Report only phase transitions, frozen inventories, verified closure, gates/holds, and terminal outcome. Include exact plan/revisions, active/released lease, decisions, coverage/docs state, capsule-payload metrics, handoffs, credits/counters, and controller `next_action`.
