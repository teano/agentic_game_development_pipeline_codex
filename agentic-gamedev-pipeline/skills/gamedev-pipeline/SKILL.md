...
1. Before `init`, require current approved requirements, specification, and development plan files. Pass them under the exact keys `requirements`, `specification`, and `plan`. Also pass one or more ordered JSON slice records with exactly `id`, `allowed_paths`, and `planned_commands`; callers never provide `read_paths`. The controller parses each approved plan slice's ordered `Owned Paths`, requires exact byte/order equality with its `Scope Contract.editable_paths` and caller `allowed_paths`, and rejects narrowing, broadening, duplicates, invalid paths, or reordering. It separately parses Context Capsule `authority_paths` plus `evidence_paths`, validates and deduplicates them, requires every disjoint `Expected Paths` entry to be covered, and stores the resulting fourth `read_paths` field. The run ID is one compact ASCII identifier, not a path: it starts and ends alphanumeric, contains at most 64 letters, digits, dots, underscores, or hyphens, and contains no separator.
